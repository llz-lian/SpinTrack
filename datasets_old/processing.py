    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['template_images'], data['template_anno'] = self.transform['joint'](
                image=data['template_images'], bbox=data['template_anno'], new_roll=False)
        ...
        for s in ['template','search']:
            ...
            # anno= boxes[0].int()
            # # if s == 'template':
            # #     anno= data[s + '_anno'][1].int()

            # xmin,ymin,w,h = anno[0].item(),anno[1].item(),anno[2].item(),anno[3].item()
            # pt1 = (xmin,ymin)
            # pt2 = (xmin + w,ymin + h)
            # old_img = cv2.rectangle(old_img,pt1,pt2,color=(0,255,255))
            # cv2.imshow("test",old_img)
            # cv2.waitKey(0)
...
class OnlineProcessing(BaseProcessing):
    """ The processing class used for training. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __init__(self, search_area_factor, template_area_factor, search_sz, temp_sz, center_jitter_factor, scale_jitter_factor,
                 mode='pair', online_area_factor = 4,online_sz = 384,*args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region relative to the target size.
            template_area_factor - The size of the template region relative to the template target size.
        ...
        self.search_area_factor = search_area_factor
        self.template_area_factor = template_area_factor
        self.online_area_factor = online_area_factor
        self.search_sz = search_sz
        self.temp_sz = temp_sz
        self.online_sz = online_sz
        self.center_jitter_factor = center_jitter_factor
        self.scale_jitter_factor = scale_jitter_factor
        self.mode = mode
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['online_images'],data['online_anno'] = self.transform['joint'](image = data['online_images'],bbox = data['online_anno'],new_roll = False)
        for s in ['search', 'template', 'online']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num search/template frames must be 1"
            ...
            if s == 'search':
                ...
                crops, boxes, _ = prutils.jittered_center_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                           self.online_area_factor, self.online_sz)
            else:
                raise NotImplementedError
        ...
        data['search_images'] = data['search_images'].squeeze()
        data['template_anno'] = data['template_anno'].squeeze()
        data['online_anno'] = data['online_anno'].squeeze()
        data['search_anno'] = data['search_anno'].squeeze()
        return data