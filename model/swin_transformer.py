...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False,checkpoint = True,down_sample = True, keep_dim = False,**kwargs):
        super().__init__()
    ...
    #    self.num_classes = num_classes
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = BasicLayer(dim=int(embed_dim * 2 ** i_layer) if keep_dim == False else embed_dim,
                                ...
                                norm_layer=norm_layer,
                                downsample=PatchMerging if (i_layer < self.num_layers - 1) and down_sample else None,
    ...
    def forward(self, x,search = None):
        # x: [B, L, C]
        if search is not None:
            search,H,W = self.patch_embed(search)
...
def buildSwin(in_channel,patch_size,window_size,embed_dim,depths,num_heads,downsample,keep_dim = False,checkpoint = True):
    ...
    return model
def buildAll(temp_in_channel,use_checkpoint,search_in_channel = 3,embed_dim = 96,num_heads = (8,8,12,24),depths = (2,2,4),window_size = 7):
    temp_swin = buildSwin(in_channel=temp_in_channel,
                          patch_size=2,
                          window_size=7,
                          embed_dim=embed_dim,
                          depths=depths,
...
def buildBase(temp_in_channel,use_checkpoint):
    return buildAll(temp_in_channel=temp_in_channel,search_in_channel=3,embed_dim=96,num_heads=(3,6,12),depths=(2,2,4),use_checkpoint=use_checkpoint)

def buildSmall(temp_in_channel,use_checkpoint):
    return buildAll(temp_in_channel=temp_in_channel,search_in_channel=3,embed_dim=72,num_heads=(4,8,12),depths=(2,2,4),use_checkpoint=use_checkpoint)#64,384 121,384

def buildLarge(temp_in_channel,use_checkpoint):
    return buildAll(temp_in_channel=temp_in_channel,search_in_channel=3,embed_dim=128,num_heads=(4,8,16),depths=(2,2,6),use_checkpoint=use_checkpoint,window_size = 12)
...
if __name__ == "__main__":
    temp_swin,search_swin = buildLarge(3,use_checkpoint=False)
    img = torch.rand([1,3,256,256])#1,484,288
    tmp = torch.rand([1,3,128,128])#1,256,288

    output = temp_swin(tmp)
    output = search_swin(img)
    print(output.shape)
