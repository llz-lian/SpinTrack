...
from .convhead import Head,DoubleHead,ClsHead,Centerhead
...
class Cross(nn.Module):
    ...
    def forward(self,features,target):
        ...
        target = target +self.cross(self.norm(target),features,features)[0]
        return target
...
class My(nn.Module):
    def __init__(self,
                 ...
                 use_chead = False,
                 use_roll = True,
                 use_checkpoint = True,roll_kv = False) -> None:
        super().__init__()
        
        assert not (use_chead and use_dhead)
        ...
        self.act = nn.Sigmoid()
        self.feature = None
        self._update_name(depth,embed_dim,relative_bias,useV2,use_conv_bias,two_cross,drop_target,few_ffn,pre_cross,change_dim,use_dhead,use_chead,use_roll,roll_kv)
        ...
        for i_layer in range(length):
            ...
            layers = ForwardBlock(
               ...
               useV2= useV2,
               use_checkpoint = (i_layer%2==0 and use_checkpoint) ,change_dim=(i_layer == length - 2 and change_dim),two_cross=two_cross,few_ffn=few_ffn,
               use_conv_bias=use_conv_bias,use_roll = use_roll,roll_kv = roll_kv)
            if i_layer==length-2 and change_dim:
                tl = tl//2
                tal = tal//2
        ...
        if not use_dhead:
            ...
            self.encoder_head2 = ClsHead(embed_dim)
            self.encoder_head = ClsHead(dim)#ConvHead(dim,target_len)
        else:
            self.head = DoubleHead(dim,conv_dim = conv_dim)#ConvHead(dim,target_len)
            self.head2 = DoubleHead(embed_dim,conv_dim = conv_dim)
            self.encoder_head2 = ClsHead(embed_dim)
            self.encoder_head = ClsHead(dim)#ConvHead(dim,target_len)
        if use_chead:
            self.head = Centerhead(dim)#ConvHead(dim,target_len)
            self.head2 = Centerhead(embed_dim)
            self.encoder_head2 = ClsHead(embed_dim)
            self.encoder_head = ClsHead(dim)#ConvHead(dim,target_len)
...
#         self.oneTtwo = nn.Linear(1,2)
        ...
        self.decoder_len = 0
        self.encoder_len = 0
        self.forward_cnt = int(0)
        self.return2 = True
        self.not_all = False
    def forward_one(self,temp,target,online = None):
    ...
    def forward(self,temp,target,online = None):
        ...
        if self.pre_cross is not None:
            target = self.pre_cross(temp,target)
        
        B,num_token,embed_dim = temp.shape
        # temp = self._forward_online(temp,online)
        if self.not_all:
            self.forward_cnt += 1 
        features = torch.cat([temp,target],dim=1)
        ...
        outputs1 = []
        outputs2 = []
        decoder_target_index = None
        encoder_target_index = None
        two = True
        for layer in self.tail_layers:
            features,out = layer(features,out)
            _,output2 = self._fowardHead(features[:,num_token:,:],out,return2 = True,return1 = False,two = two)
            if self.not_all:
                if feature_len == features.shape[1]:
                    maps =  output2['pred_logits'][:,:,0]#B,N
                    _, indices = torch.sort(maps,dim = -1,descending=True)
                    decoder_target_index = indices[:,0:num_token].tolist()
                    encoder_target_index = decoder_target_index
                else:
                    maps =  output2['pred_logits'][:,:,0]
                    _, indices = torch.sort(maps,dim = -1,descending=True)
                    encoder_target_index = indices[:,0:num_token].tolist()

            out,_,_ = layer.forward_cross(features,out,decoder_target_index,encoder_target_index)

            output1,_ = self._fowardHead(features[:,num_token:,:],out,return2 = False,return1 = True,two = two)

            outputs1 = outputs1 + [output1]
        ...
        # out2 = self.norm2(target)
        # classes2,box2,_ = self.head(out2)
        # output2 = {'pred_boxes':box2,'pred_logits':classes2,'gaussian':0}
        # output = {'pred_boxes':box,'pred_logits':classes,'gaussian':0}
        return outputs1 ,outputs2

    def _fowardHead(self,target,out,return2 = True,return1 = True,two = False):
        B,N,C = out.shape
        head = self.head
        norm = self.norm
        ...
        if C == self.embed_dim:
            head = self.head2
            encoder_head = self.encoder_head2
            norm = self.norm2
            encoder_norm = self.encoder_norm2
        output = []
        if return1:
            out = norm(out)
        ...
        output2 = []
        if return2:
            out2 = encoder_norm(target)
            classes2 = encoder_head(out2)
#             classes2 = self.oneTtwo(classes2)
            output2 = {'pred_logits':classes2,'gaussian':0}
        return output,output2
    
    def set_not_all(self):
        self.not_all = True
        print('not_all set True')
    ...
    def track(self,target):
        ...
        for layer in self.tail_layers:
            ...
            _,output2 = self._fowardHead(features[:,num_token:,:],out,return2 = True,return1 = False,two = two)
            if self.not_all:
                if decoder_target_index is None:
                    maps =  output2['pred_logits'][:,:,0]#B,N
                    _, indices = torch.sort(maps,dim = -1,descending=True)
                    decoder_target_index = indices[:,0:num_token].tolist()
                    encoder_target_index = decoder_target_index
...
#             two = False
    ...
    def _update_name(self,depth,embed_dim,relative_bias,useV2,use_conv_bias,two_cross,drop_targ,few_ffn,pre_cross,change_dim,use_dhead,use_chead,use_roll,roll_kv):
...
def GetMyBase(use_checkpoint,
    ...
    model.str = model.str + '_base'
    return model

def GetMyBase12(use_checkpoint,
              relative_bias,
              use_conv_bias,
              change_dim,
              two_cross,
    ...
    model =  My(depth=4,embed_dim=384,num_heads=12,mlp_ratio=4,drop_ratio=0.2,use_checkpoint = use_checkpoint,
    temp_len=12,target_len=24,funcBuild=getbase,
                relative_bias =relative_bias,
                use_conv_bias =use_conv_bias,
                change_dim =change_dim,
    ...
    model.str = model.str + '_h12_base'
    return model


def GetMyBaseL(use_checkpoint,
              relative_bias,
              use_conv_bias,
              change_dim,
              two_cross,
...
def GetMyLarge(use_checkpoint,
              ...
              roll_kv):
    num_heads = 16
    model = My(depth=4,embed_dim=512,num_heads=num_heads,mlp_ratio=4,drop_ratio=0.1,use_checkpoint = use_checkpoint,
    ...
    model.str = model.str+'_'+str(num_heads)+'_large.pth'
    return model

def GetMyLarge384(use_checkpoint = False,
              relative_bias = True,
              use_conv_bias = True,
              change_dim = True,
              two_cross = False,
              ...
              use_roll = False,
              roll_kv = False):
    model = My(depth=4,embed_dim=512,num_heads=16,mlp_ratio=4,drop_ratio=0.1,use_checkpoint = use_checkpoint,
    temp_len=12,target_len=24,funcBuild=getlarge384,
              relative_bias =relative_bias,
                ...
                pre_cross =pre_cross,
                use_dhead = use_dhead)
    model.str = model.str+'_16_large384'
    return model
